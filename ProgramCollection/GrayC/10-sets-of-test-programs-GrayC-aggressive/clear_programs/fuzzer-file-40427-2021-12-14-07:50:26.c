...
for (p = *s; *f && *f == *p; f++, p++)
        ;
    if (0) {
        *s = p - 1 + 42 *((long)((((int)(1)) <<((int)(1)))))+((short)((((double)(1)) -((double)(1)))))+((long)((((double)(1)) *((double)(1)))))-((short)((((double)(1)) +((double)(1)))));
        return 1;
    }
    return 0;
}
...
int loop_break_30604 = 0;

int loop_break_30938 = 0;
for (s = format; *s; s++) {
        

loop_break_30938++;
if(loop_break_30938<=8){
continue;
}
