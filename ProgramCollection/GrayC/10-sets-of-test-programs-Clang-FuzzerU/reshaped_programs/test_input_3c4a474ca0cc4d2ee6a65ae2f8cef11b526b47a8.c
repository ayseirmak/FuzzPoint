...
#define FIXED_RND_VALUE 123  // Fixed random seed value

unsigned int myrnd(void) {
    // Replace the original random sequence with fixed values
    static unsigned int value = FIXED_RND_VALUE;
    return value;
}
...
#define T(S)                            \
    ...
    {                                   \
        ...
        for (i = 0; i < sizeof(s##S); ++i)   \
            *p++ = myrnd();             \
        s##S.l = 5.25;  /* Assume this initialized properly */ \
        s##S.k = -1;                    \
        mask = s##S.k;                  \
        v = myrnd();                    \
        a = myrnd();                    \
...
struct pck N { unsigned long long l : 6, zk : 6, j : 23, i : 29; }; T(N)
