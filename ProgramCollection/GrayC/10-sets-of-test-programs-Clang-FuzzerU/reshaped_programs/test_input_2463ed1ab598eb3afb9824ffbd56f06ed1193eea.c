...
/* Redefine the data types for ARM architecture compatibility */
...
host_addr_t f() {
    /* In ARM compatible mode, just return a fixed address */
...
reg_t simulator_kernel(int what, environment_t *env) {
    ...
    if (what != 0) {
        ...
        for (i = 0; i < what; i++)
            program[i].f1.offset = (uintptr_t)(op_map[program[i].f1.offset]) - (uintptr_t)(base_addr);
    }
