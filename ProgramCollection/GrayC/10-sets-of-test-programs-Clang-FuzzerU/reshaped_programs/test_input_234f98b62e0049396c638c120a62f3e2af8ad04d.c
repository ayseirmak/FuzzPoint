...
\
void test##S(unsigned int v, unsigned int a) \
{ \
  int i; \
  unsigned int mask, r; \
  struct S x; \
  ...
  for (i = 0; i < sizeof(s##S); ++i) \
    *p++ = i + 1; /* Initializing with deterministic value */ \
...
int main(void)
{
  unsigned int fixed_v = 1000; /* Fixed value */
  unsigned int fixed_a = 50;   /* Fixed value */

  testA(fixed_v, fixed_a);
  testB(fixed_v, fixed_a);
  testC(fixed_v, fixed_a);
  testD(fixed_v, fixed_a);
  testE(fixed_v, fixed_a);
  testF(fixed_v, fixed_a);
  testG(fixed_v, fixed_a);
  testH(fixed_v, fixed_a);
  testI(fixed_v, fixed_a);
  testJ(fixed_v, fixed_a);
  testK(fixed_v, fixed_a);
  testL(fixed_v, fixed_a);
  testM(fixed_v, fixed_a);
  testN(fixed_v, fixed_a);
  testO(fixed_v, fixed_a);
  testP(fixed_v, fixed_a);
  testQ(fixed_v, fixed_a);
  testR(fixed_v, fixed_a);
  testS(fixed_v, fixed_a);
  testT(fixed_v, fixed_a);
  testU(fixed_v, fixed_a);
  testV(fixed_v, fixed_a);
  testW(fixed_v, fixed_a);
  testX(fixed_v, fixed_a);
  testY(fixed_v, fixed_a);
  testZ(fixed_v, fixed_a);
