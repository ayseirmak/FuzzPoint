...
// Define fixed seed values for deterministic behavior
static unsigned int my_seed = 1388815473;

// Replacing the `myrnd` function with deterministic sequence generation
unsigned int myrnd_fixed(void) {
    my_seed *= 1103515245;
    my_seed += 12345;
    return (my_seed / 65536) % 2048;
}

// Macro definition remains unchanged
#define T(S)                                  \
struct S s##S;                                \
...
// Define struct with attribute packed
...
struct pck W { long double l; unsigned int k : 40, j : 13, i : 7; }; T(W)
