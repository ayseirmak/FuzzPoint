...
void arg(void) {
    printf("Arguments:\n");
    fa_s1(s1); fa_s2(s2); fa_s3(s3); fa_s4(s4); fa_s5(s5); fa_s6(s6); fa_s7(s7); fa_s8(s8);
    fa_s9(s9); fa_s10(s10); fa_s11(s11); fa_s12(s12); fa_s13(s13); fa_s14(s14); fa_s15(s15);
...
void movi(void){
    ...
    unsigned long long nums[] = {
        0, 0xabcd, 0xabcd0000, 0xabcd00000000, 0xabcd000000000000, 0xffffabcd, 0xabcdffff,
        0xffffffffffffabcd, 0xffffffffabcdffff, 0xffffabcdffffffff, 0xabcdffffffffffff,
        0xaaaaaaaa, 0x5555555555555555, 0x77777777, 0x3333333333333333, 0xf8f8f8f8,
        0x1e1e1e1e1e1e1e1e, 0x3f803f80, 0x01ff01ff01ff01ff, 0x007fffc0, 0x03fff80003fff800,
        0x0007fffffffffe00, 0xabcd1234, 0xabcd00001234, 0xabcd000000001234, 0xabcd12340000,
        0xabcd000012340000, 0xabcd123400000000, 0xffffffffabcd1234, 0xffffabcdffff1234,
        0xabcdffffffff1234, 0xffffabcd1234ffff, 0xabcdffff1234ffff, 0xabcd1234ffffffff,
        0xffffef0123456789, 0xabcdef012345ffff, 0xabcdef0123456789
    };
...
void opi(void) {
    int x = 1000;
    pll(addip0(x)); pll(sublp0(x)); pll(addip123(x)); pll(addlm123(x));
    pll(sublp4095(x)); pll(subim503808(x)); pll(addp12345(x)); pll(subp12345(x));
    pll(mvni(x)); pll(negl(x)); pll(rsbi123(x)); pll(rsbl123(x)); pll(andi0(x));
    pll(andlm1(x)); pll(orrl0(x)); pll(orrim1(x)); pll(eori0(x)); pll(eorlm1(x));
    pll(and0xf0(x)); pll(orr0xf0(x)); pll(eor0xf0(x)); pll(lsli0(x)); pll(lsri0(x));
    pll(asrl0(x)); pll(lsli1(x)); pll(lsli31(x)); pll(lsll1(x)); pll(lsll63(x));
    pll(lsri1(x)); pll(lsri31(x)); pll(lsrl1(x)); pll(lsrl63(x)); pll(asri1(x));
    pll(asri31(x)); pll(asrl1(x)); pll(asrl63(x));
}
