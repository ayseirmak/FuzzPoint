...
// Define structures with initializations
...
// Define function to print structures
...
// Define additional utility functions and function for operations

// Example operation
static uint32_t addip0(uint32_t x) { return x + 0; }
static uint32_t subip0(uint32_t x) { return x - 0; }
static uint32_t mvni(uint32_t x) { return ~x; }
static uint64_t negl(uint64_t x) { return ~x + 1; }

void pll(uint64_t value) {
    printf("0x%016llx\n", value);
}
