...
// Simplified fixed value generator to replace myrnd() function with deterministic values
unsigned int get_fixed_value(size_t index) {
    // Arbitrary deterministic sequence of values
    static unsigned int values[] = { 123, 456, 789, 1011, 1234, 5678, 9101, 2345, 6789, 10112, 13141, 51617, 18191, 20212 };
...
void test##S(void) {                                    \
    ...
    for (i = 0; i < sizeof(s##S); ++i)                   \
        *p++ = get_fixed_value(i);                       \
    s##S.l = (__builtin_classify_type(s##S.l) == 8) ? 5.25 : s##S.l; \
    s##S.k = -1;                                         \
    mask = s##S.k;                                       \
    v = get_fixed_value(5);                              \
    a = get_fixed_value(6);                              \
...
struct pck T { unsigned short k:2, j:8, i:7; unsigned short l; }; T(T)
