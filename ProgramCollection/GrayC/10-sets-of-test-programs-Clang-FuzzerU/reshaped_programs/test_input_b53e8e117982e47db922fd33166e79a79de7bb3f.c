...
struct G { unsigned int i : 12, j : 13, k : 7; };
struct H { unsigned int i : 12, j : 11, k : 9; };
struct I { unsigned short i : 1, j : 6, k : 9; };
...
struct Q { unsigned int k : 12, j : 13, i : 7; };
struct R { unsigned int k : 12, j : 11, i : 9; };
struct S { unsigned short k : 1, j : 6, i : 9; };
struct T { unsigned short k : 1, j : 8, i : 7; unsigned short l; };
struct U { unsigned short j : 6, k : 1, i : 9; };
...
#define STATIC_SEED 1388815473

unsigned int
deterministic_rnd (void)
{
    static unsigned int s = STATIC_SEED;
    s *= 1103515245;
    s += 12345;
    return (s / 65536) % 2048;
}
...
unsigned int fn1##S (unsigned int x) { struct S y = s##S; y.k += x; y = retme##S(y); return y.k; } \
unsigned int fn2##S (unsigned int x) { struct S y = s##S; y.k += x; y.k %= 15; return y.k; } \
unsigned int retit##S (void) { return s##S.k; } \
unsigned int fn3##S (unsigned int x) { s##S.k += x; return retit##S(); } \
void test##S (void) { \
    int i; unsigned int mask, v, a, r; \
    struct S x; \
    char *p = (char *) &s##S; \
    for (i = 0; i < sizeof (s##S); ++i) *p++ = deterministic_rnd(); \
