...
/* Declare deterministic 'random' numbers for testing purposes */
unsigned int deterministic_myrnd_values[] = {1000, 2000, 1500, 1400, 1600, 1700, 1800};
int myrnd_index = 0;

unsigned int myrnd(void) {
    unsigned int value = deterministic_myrnd_values[myrnd_index];
    myrnd_index = (myrnd_index + 1) % (sizeof(deterministic_myrnd_values) / sizeof(unsigned int));
    return value;
}
...
struct L { unsigned int k : 5, j : 11, i : 15; unsigned int l; }; T(L)
