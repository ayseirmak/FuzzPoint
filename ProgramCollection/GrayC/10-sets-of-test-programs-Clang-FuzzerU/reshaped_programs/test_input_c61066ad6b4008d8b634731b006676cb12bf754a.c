...
unsigned int deterministic_value(void) {
    return DEFAULT_VALUE; // Fixed deterministic value instead of randomness
}
...
{                                                       \
    ...
    struct S x;                                         \
    volatile char *p = (volatile char *) &s##S;         \
    for (i = 0; i < (int)sizeof(s##S); ++i)             \
        *p++ = deterministic_value();                   \
    if (__builtin_classify_type(s##S.l) == 8)           \
        s##S.l = 5.25;                                  \
    s##S.k = (unsigned int)-1;                          \
    mask = s##S.k;                                      \
    v = deterministic_value();                          \
    a = deterministic_value();                          \
    s##S.k = v;                                         \
    x = s##S;                                           \
    r = fn1##S(a);                                      \
...
struct pck R { unsigned short k : 1, j : 11, i : 3; unsigned long long l; }; T(R)
