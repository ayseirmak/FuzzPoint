...
#define NUM_FIXED_VALUES 6
static const unsigned int fixed_values[NUM_FIXED_VALUES] = {23, 42, 17, 93, 56, 81};
static int fixed_index = 0;

unsigned int myrnd(void) {
    // Return a fixed deterministic value from an array
    unsigned int value = fixed_values[fixed_index];
    fixed_index = (fixed_index + 1) % NUM_FIXED_VALUES;
    return value;
}
...
/* Definitions for various structures with bitfields */
...
struct H { unsigned int i : 15, j : 11, k : 6; unsigned int l; }; T(H)
struct I { unsigned int l; unsigned int i : 15, j : 11, k : 6; }; T(I)
struct J { unsigned long long l : 6, i : 29, j : 23, k : 6; }; T(J)
struct K { unsigned long long l, i : 12, j : 29, k : 23; }; T(K)
struct L { unsigned long long l, i : 29, j : 23, k : 12; }; T(L)
