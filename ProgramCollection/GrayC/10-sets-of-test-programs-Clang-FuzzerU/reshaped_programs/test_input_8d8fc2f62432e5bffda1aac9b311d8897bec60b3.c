...
// Define fixed deterministic values to substitute the dynamic myrnd function
#define FIXED_VALUE_1 1234
#define FIXED_VALUE_2 5678
...
{                              \
  ...
  x = s##S;                    \
  r = fn2##S(a);               \
  assert(x.i == s##S.i && x.j == s##S.j && x.k == s##S.k && ((((v + a) & mask) % 15) & mask) == r); \
  v = FIXED_VALUE_1;           \
  a = FIXED_VALUE_2;           \
  s##S.k = v;                  \
  x = s##S;                    \
  r = fn3##S(a);               \
  assert(x.i == s##S.i && x.j == s##S.j && s##S.k == r && ((v + a) & mask) == r); \
}

// Define structures with bit fields
...
struct H { unsigned int i : 7; unsigned short l; }; T(H)
struct I { unsigned int i : 8, j : 12, k : 11; unsigned long long l; }; T(I)
struct J { unsigned int i : 7; unsigned short l; }; T(J)
