...
// Simulated random number generator function
unsigned int myrnd(void) {
    static unsigned int sequence[10] = {526, 1023, 1500, 1002, 874, 606, 492, 744, 999, 456};
    static int index = 0;
    unsigned int result = sequence[index];
    index = (index + 1) % 10;
    return result;
}

// Macro definition to generate structures and functions for each test
#define T(S)                                  \
struct S s##S;                                \
