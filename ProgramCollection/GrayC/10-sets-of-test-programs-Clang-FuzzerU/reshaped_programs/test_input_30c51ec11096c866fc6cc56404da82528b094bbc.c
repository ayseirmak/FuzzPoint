...
{                                                   \
  ...
  for (i = 0; i < sizeof (s##S); ++i)               \
    *p++ = 42; /* Replaced myrnd() with fixed value 42 */ \
  s##S.l = 5.25;                                    \
  s##S.k = -1;                                      \
  mask = s##S.k;                                    \
  v = 42; /* Replaced myrnd() with fixed value 42 */\
  a = 42; /* Replaced myrnd() with fixed value 42 */\
  s##S.k = v;                                       \
  x = s##S;                                         \
  r = fn1##S (a);                                   \
  if (x.i != s##S.i || x.j != s##S.j                \
      ...
      || ((v + a) & mask) != r)                     \
    abort ();                                       \
  v = 42; /* Repeated for consistency */            \
  a = 42; /* Repeated for consistency */            \
  s##S.k = v;                                       \
  x = s##S;                                         \
  r = fn2##S (a);                                   \
  if (x.i != s##S.i || x.j != s##S.j                \
