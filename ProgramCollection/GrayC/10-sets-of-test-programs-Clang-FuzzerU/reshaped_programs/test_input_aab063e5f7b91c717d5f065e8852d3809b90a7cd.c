...
/* Initialize deterministic values instead of using randomness */
unsigned char deterministic_values[] = {5, 10, 15, 20, 25, 30, 35, 40, 45, 50};
...
{                                        \
  ...
  for (i = 0; i < sizeof (s##S); ++i)    \
    *p++ = deterministic_values[i % (sizeof(deterministic_values)/sizeof(deterministic_values[0]))]; \
  if (__builtin_classify_type (s##S.l) == 8)    \
    s##S.l = 5.25;                       \
  s##S.k = -1;                           \
  mask = s##S.k;                         \
  v = deterministic_values[1];           \
  a = deterministic_values[2];           \
  s##S.k = v;                            \
  x = s##S;                              \
  r = fn1##S (a);                        \
  ...
  v = deterministic_values[3];           \
  a = deterministic_values[4];           \
  s##S.k = v;                            \
  x = s##S;                              \
  r = fn1##S (a);                        \
  if (x.i != s##S.i || x.j != s##S.j     \
      ...
      || ((((v + a) & mask) % 15) & mask) != r) \
    printf("Test failed in " #S "\n");   \
  v = deterministic_values[5];           \
  a = deterministic_values[6];           \
  s##S.k = v;                            \
  x = s##S;                              \
  r = fn3##S (a);                        \
  if (x.i != s##S.i || x.j != s##S.j     \
...
struct U { unsigned short j : 3, k : 1, i : 9; unsigned long long l; }; T(U)
