...
// Function to simulate deterministic 'myrnd' with fixed values for testing
unsigned int myrnd(void) {
    static unsigned int values[] = { 1000, 800, 600, 400, 200, 
...
void test##S (void) { \
    ...
    if (sizeof(s##S.l) > sizeof(unsigned int)) { \
        s##S.l = 5.25; \
    } \
