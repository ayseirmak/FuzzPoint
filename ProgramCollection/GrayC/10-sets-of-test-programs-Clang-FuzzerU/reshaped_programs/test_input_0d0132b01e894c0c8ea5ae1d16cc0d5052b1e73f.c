...
// Define a deterministic sequence of numbers instead of using randomness
unsigned int deterministic_number_sequence[] = {
    1234, 5678, 91011, 121314, 151617, 181920, 212223,
    242526, 272829, 303132, 333435, 363738, 394041
};
int sequence_index = 0;

unsigned int myrnd(void) {
    unsigned int num = deterministic_number_sequence[sequence_index];
    sequence_index = (sequence_index + 1) % (sizeof(deterministic_number_sequence) / sizeof(unsigned int));
    return num;
}

// Macro to define structures and related functions
#define T(S) \
struct S s##S = {0}; \
...
void test##S(void) { \
    ...
    struct S x; \
    char *p = (char *)&s##S; \
    for (size_t i = 0; i < sizeof(s##S); ++i) *p++ = (char)myrnd(); \
...
struct X { unsigned int k : 12, j : 13, i : 6; long double l; }; T(X)
