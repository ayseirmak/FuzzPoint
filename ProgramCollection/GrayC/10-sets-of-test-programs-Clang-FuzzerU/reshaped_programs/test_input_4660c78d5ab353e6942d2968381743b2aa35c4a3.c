...
static unsigned int predefined_values[] = {1000, 500, 2000, 1500, 1200, 2500}; // Deterministic set of values for testing
static int predetermined_cycle = 0;

unsigned int get_deterministic_value(void) {
    unsigned int value = predefined_values[predetermined_cycle];
    predetermined_cycle = (predetermined_cycle + 1) % (sizeof(predefined_values)/sizeof(predefined_values[0]));
    return value;
}
...
unsigned int fn3##S(unsigned int x) {     \
    ...
    return retit##S();                    \
                                          ...
                                          \
void test##S(void) {                      \
    unsigned int i, mask, v, a, r;        \
    struct S x;                           \
    char *p = (char *)&s##S;              \
    for (i = 0; i < sizeof(s##S); ++i) {  \
        *p++ = get_deterministic_value(); \
    }                                     \
    s##S.k = (unsigned int)-1;            \
    mask = s##S.k;                        \
    v = get_deterministic_value();        \
    a = get_deterministic_value();        \
    s##S.k = v;                           \
    x = s##S;                             \
    r = fn1##S(a);                        \
