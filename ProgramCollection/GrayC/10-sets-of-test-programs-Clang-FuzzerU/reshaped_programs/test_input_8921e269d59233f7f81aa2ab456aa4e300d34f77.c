...
double foo(double x) {
    ...
    case 22:
        t = 2 * x - 45;
        return 0.40842e-1 + (0.11650e-2 + (0.72569e-5 + (0.42796e-0.45379e-6 + (0.31874e-8 + (0.18277e-10 + 0.83144e-13 * t) * t) * t) * t) * t) * t;
    case 69:
        t = 2 * x - 139;
...
int main() {
    double d = foo(78.0); // Fixed value
    assert(d >= 0.38 && d <= 0.42); // Ensures the result is in the expected range
    return 0;
}