...
/* Define a fixed sequence of "random" numbers to ensure deterministic behavior */
static unsigned int predetermined_values[] = {
    1023, 2047, 512, 1024, 256, 2048, 128, 512, 64, 32,
    16, 8, 4, 2, 1, 0, 100, 200, 300, 400, 500
};
static int value_index = 0;

unsigned int myrnd(void) {
  if (value_index < sizeof(predetermined_values) / sizeof(predetermined_values[0])) {
    return predetermined_values[value_index++];
  } else {
    return 0;  // Return 0 if we run out of predefined values
  }
}
...
void test##S(void) {                          \
  ...
  x = s##S;                                   \
  r = fn1##S(a);                              \
  assert(x.i == s##S.i && x.j == s##S.j && x.k == s##S.k && (((v + a) & mask) == r)); \
  a = myrnd();                                \
  s##S.k = v;                                 \
  x = s##S;                                   \
  r = fn1##S(a);                              \
  assert(x.i == s##S.i && x.j == s##S.j && x.k == s##S.k && ((((v + a) & mask) % 15) == r)); \
  a = myrnd();                                \
  s##S.k = v;                                 \
  x = s##S;                                   \
  r = fn3##S(a);                              \
  assert(x.i == s##S.i && x.j == s##S.j && ((v + a) & mask) == r); \
}
...
struct O { unsigned long long l, k : 13, j : 23, i : 29; }; T(O)
