...
void test##S(void) {                                        \
    ...
    for (i = 0; i < sizeof(s##S); ++i)                      \
        *p++ = (i & 0xFF); /* Deterministic pseudo-random */\
    if (__builtin_classify_type(s##S.l) == 8)               \
        s##S.l = 5; /* Replace use of non-standard Inf */   \
    s##S.k = (unsigned int) -1;                             \
    mask = s##S.k;                                          \
    v = 1024; /* Deterministic fixed value */               \
    a = 512; /* Deterministic fixed value */                \
    s##S.k = v;                                             \
    x = s##S;                                               \
    r = fn1##S(a);                                          \
    if (x.i != s##S.i || x.j != s##S.j                      \
        ...
        || ((v + a) & mask) != r)                           \
        abort();                                            \
    v = 2048; /* Deterministic fixed value */               \
    a = 1024; /* Deterministic fixed value */               \
    s##S.k = v;                                             \
    x = s##S;                                               \
    r = fn2##S(a);                                          \
    if (x.i != s##S.i || x.j != s##S.j                      \
        ...
        || ((((v + a) & mask) % 15) & mask) != r)           \
        abort();                                            \
    v = 4096; /* Deterministic fixed value */               \
    a = 2048; /* Deterministic fixed value */               \
    s##S.k = v;                                             \
    x = s##S;                                               \
    r = fn3##S(a);                                          \
    if (x.i != s##S.i || x.j != s##S.j                      \
...
// Define the structures with bit-fields
