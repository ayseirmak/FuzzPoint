...
/* Define Fixed "Random" Values */
unsigned int myrnd(void) {
    return 123;  // Fixed value for deterministic behavior
}
...
void test##S(void) {                                         \
    ...
    for (i = 0; i < sizeof(s##S); ++i)                       \
        *p++ = (char)myrnd();                                \
    if (1) /* If condition simplified for determinism */     \
        s##S.l = 5.25;                                       \
    s##S.k = 0xFFFFFFFF;                                     \
    mask = s##S.k;                                           \
    v = myrnd();                                             \
    ...
    r = fn2##S(a);                                           \
    assert(x.i == s##S.i && x.j == s##S.j && x.k == s##S.k &&\
           x.l == s##S.l && ((((v + a) & mask) % 15) & mask) == r); \
    v = myrnd();                                             \
    a = myrnd();                                             \
    s##S.k = v;                                              \
    x = s##S;                                                \
    r = fn3##S(a);                                           \
    assert(x.i == s##S.i && x.j == s##S.j && x.k == r && x.l == s##S.l);\
}
