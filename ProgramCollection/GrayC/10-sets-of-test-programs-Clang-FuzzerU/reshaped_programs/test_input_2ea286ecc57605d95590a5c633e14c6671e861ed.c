...
struct b_ldouble { short s; long double ld; } s_s_ld = { 28, 29.0L };
struct c_ldouble { int i; long double ld; } s_i_ld = { 30, 31.0L };
struct e_ldouble { double d; long double ld; } s_d_ld = { 34.0, 35.0L };
struct f_ldouble { float f; long double ld; } s_f_ld = { 32.0f, 33.0L };
...
int main() {
    ...
    if (s_f_d.f != 25.0f) { abort(); }
    if (s_f_d.d != 26.0) { abort(); }
    
    if (s_c_ld.c != 'e') { abort(); }
    if (s_c_ld.ld != 27.0L) { abort(); }
    
    if (s_s_ld.s != 28) { abort(); }
    if (s_s_ld.ld != 29.0L) { abort(); }
    
    if (s_i_ld.i != 30) { abort(); }
    if (s_i_ld.ld != 31.0L) { abort(); }
    
    if (s_f_ld.f != 32.0f) { abort(); }
    if (s_f_ld.ld != 33.0L) { abort(); }
    
    if (s_d_ld.d != 34.0) { abort(); }
    if (s_d_ld.ld != 35.0L) { abort(); }
