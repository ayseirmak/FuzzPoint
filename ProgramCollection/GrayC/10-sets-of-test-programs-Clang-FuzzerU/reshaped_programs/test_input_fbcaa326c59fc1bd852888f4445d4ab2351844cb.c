...
// Defining macros to use within the program
#define T(S) \
struct S s##S; \
...
void test##S(void) \
{ \
  unsigned int mask = 0xFF; /* Fixed deterministic value */ \
  unsigned int v = 1234; /* Fixed deterministic value */ \
  unsigned int a = 5678; /* Fixed deterministic value */ \
  unsigned int r; \
  struct S x; \
  char *p = (char *)&s##S; \
  for (int i = 0; i < sizeof(s##S); ++i) \
    *p++ = 1; /* Deterministic initialization */ \
  if (sizeof(s##S.l) == sizeof(float)) \
    s##S.l = 5.25; \
  s##S.k = -1; \
  mask = s##S.k; \
  s##S.k = v; \
  ...
  if (x.i != s##S.i || x.j != s##S.j \
      ...
      || ((v + a) & mask) != r) \
    abort(); \
  v = 2345; /* Fixed deterministic value */ \
  a = 6789; /* Fixed deterministic value */ \
  s##S.k = v; \
  x = s##S; \
  r = fn1##S(a); \
  if (x.i != s##S.i || x.j != s##S.j \
      ...
      || ((((v + a) & mask) % 15) & mask) != r) \
    abort(); \
  v = 3456; /* Fixed deterministic value */ \
  a = 7890; /* Fixed deterministic value */ \
  s##S.k = v; \
  x = s##S; \
  r = fn3##S(a); \
  if (x.i != s##S.i || x.j != s##S.j \
...
struct T_s { unsigned short k : 1, j : 8, i : 7; unsigned short l; }; T(T_s)
