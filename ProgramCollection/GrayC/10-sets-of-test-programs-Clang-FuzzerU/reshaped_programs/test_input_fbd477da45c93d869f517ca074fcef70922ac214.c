...
// Function stubs for testing the behavior
void abort(void) {
    printf("Abort called\n");
    exit(1);
}
...
void exit(int status) {
    ...
    // In actual use, you can call stdlib exit function
    // Uncomment the line below for actual exit call
    // _exit(status);
}

#define STATIC_INITIAL_VALUE 42 // arbitrary fixed value to replace random numbers
...
void test##S(void) { \
    int i = 0; \
    unsigned int mask = 0, v = STATIC_INITIAL_VALUE, a = STATIC_INITIAL_VALUE, r = 0; \
    struct S x; \
    char *p = (char *)&s##S; \
    for (i = 0; i < sizeof(s##S); ++i) { *p++ = STATIC_INITIAL_VALUE; } \
    if (__builtin_classify_type(s##S.l) == 8) { s##S.l = 5.25; } \
    s##S.k = -1; \
    mask = s##S.k; \
    v = STATIC_INITIAL_VALUE; \
    a = STATIC_INITIAL_VALUE; \
    s##S.k = v; \
    x = s##S; \
    r = fn1##S(a); \
