...
/* Deterministic pseudo-random number generator for fixed values */
unsigned int deterministic_random(void) {
    static unsigned int index = 0;
    static unsigned int values[] = {1700, 2045, 1023, 33, 1777};
    unsigned int size = sizeof(values) / sizeof(values[0]);
    unsigned int value = values[index % size];
    index++;
    return value;
}
...
{                                                    \
    ...
    for (i = 0; i < sizeof(s##S); ++i)               \
        *p++ = deterministic_random();               \
    if (sizeof(s##S.l) == sizeof(double))            \
        s##S.l = 5.25;                               \
    s##S.k = -1;                                     \
    mask = s##S.k;                                   \
    v = deterministic_random();                      \
    a = deterministic_random();                      \
    s##S.k = v;                                      \
    x = s##S;                                        \
    r = fn1##S(a);                                   \
