...
bool isnan_float(float x) { return x != x; }

static const float one_sqrt = 1.0f, tiny_sqrt = 1.0e-30;
...
float __ieee754_asinf(float x) {
    ...
    if (ix >= 0x3F79999A) {
        ...
        gf_u2.value = w;
        iw = gf_u2.word;
        ieee_float_shape_type sf_u2;
        sf_u2.word = (iw & 0xfffff000);
        w = sf_u2.value;
        c = (t - w * w) / (s + w);
        r = p / q;
...
int main() {
    ...
    if (!isnan_float(res)) {
        printf("Error: Expected result to be NAN, but got %f\n", res);
        return 1;
    }
