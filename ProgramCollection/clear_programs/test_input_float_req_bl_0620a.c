#include <stdio.h>
#include <assert.h>

typedef int __int32_t;
typedef unsigned int __uint32_t;

typedef union {
  float value;
  __uint32_t word;
} ieee_float_shape_type;

static const float huge_floor = 1.0e30;

float fabs_float(float x) {
  __uint32_t ix;
  ieee_float_shape_type gf_u;
  gf_u.value = (x);
  ix = gf_u.word;
  
  ieee_float_shape_type sf_u;
  sf_u.word = (ix & 0x7fffffff);
  x = sf_u.value;
  
  return x;
}

static const float atanhi_atan[] = {
    4.6364760399e-01,
    7.8539812565e-01,
    9.8279368877e-01,
    1.5707962513e+00,
};

static const float atanlo_atan[] = {
    5.0121582440e-09,
    3.7748947079e-08,
    3.4473217170e-08,
    7.5497894159e-08,
};

static const float aT_atan[] = {
    3.3333334327e-01, -2.0000000298e-01, 1.4285714924e-01, -1.1111110449e-01,
    9.0908870101e-02, -7.6918758452e-02, 6.6610731184e-02, -5.8335702866e-02,
    4.9768779427e-02, -3.6531571299e-02, 1.6285819933e-02,
};

static const float one_atan = 1.0, huge_atan = 1.0e30,
                   pi_o_4 = 7.8539818525e-01, pi_o_2 = 1.5707963705e+00,
                   pi = 3.1415927410e+00;

float atan_float(float x) {
  float w, s1, s2, z;
  __int32_t ix, hx, id;

  ieee_float_shape_type gf_u;
  gf_u.value = (x);
  hx = gf_u.word;
  ix = hx & 0x7fffffff;
  
  if (ix >= 0x50800000) {
    if (ix > 0x7f800000L)
      return x + x;
    return hx > 0 ? atanhi_atan[3] + atanlo_atan[3] : -atanhi_atan[3] - atanlo_atan[3];
  }
  
  if (ix < 0x3ee00000) {
    if (ix < 0x31000000) {
      return huge_atan + x > one_atan ? x : x;
    }
    id = -1;
  } else {
    x = fabs_float(x);
    if (ix < 0x3f980000) {
      if (ix < 0x3f300000) {
        id = 0;
        x = (2.0f * x - one_atan) / (2.0f + x);
      } else {
        id = 1;
        x = (x - one_atan) / (x + one_atan);
      }
    } else {
      if (ix < 0x401c0000) {
        id = 2;
        x = (x - 1.5f) / (one_atan + 1.5f * x);
      } else {
        id = 3;
        x = -1.0f / x;
      }
    }
  }

  z = x * x;
  w = z * z;

  s1 = z * (aT_atan[0] +
            w * (aT_atan[2] +
                 w * (aT_atan[4] +
                      w * (aT_atan[6] + w * (aT_atan[8] + w * aT_atan[10])))));
  s2 =
      w *
      (aT_atan[1] +
       w * (aT_atan[3] + w * (aT_atan[5] + w * (aT_atan[7] + w * aT_atan[9]))));
  return id < 0 ? x - x * (s1 + s2) : ((hx < 0) ? -(atanhi_atan[id] - ((x * (s1 + s2) - atanlo_atan[id]) - x)) : atanhi_atan[id] - ((x * (s1 + s2) - atanlo_atan[id]) - x));
}

int __signbit_float(float x) {
  __uint32_t w;
  ieee_float_shape_type gf_u;
  gf_u.value = (x);
  w = gf_u.word;

  return (w & 0x80000000) != 0;
}

int main() {
  float x = 0.0f;
  float res = atan_float(x);

  assert(res == 0.0f && __signbit_float(res) == 0);
  
  printf("Test passed: atan(0.0) returned %f\n", res);

  return 0;
}