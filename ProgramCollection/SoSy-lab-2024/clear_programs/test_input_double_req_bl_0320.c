...
// Union to facilitate conversion between double and two 32-bit ints.
typedef union {
  double value;
  struct {
    __uint32_t lsw;
...
static const double zero_krempio = 0.0, one_krempio = 1.0, two24_krempio = 1.67772160000000000000e+07, twon24_krempio = 5.96046447753906250000e-08;

static const int32_t two_over_pi_rempio[] = {
...
static const int32_t npio2_hw_rempio[] = {
...
static const double zero_rempio = 0.0, half_rempio = 0.5, two24_rempio = 16777216.0, invpio2_rempio = 0.63661977236758134308, pio2_1_rempio = 1.57079632673412561417, pio2_1t_rempio = 6.07710050650619224932e-11, pio2_2_rempio = 6.07710050630396597660e-11, pio2_2t_rempio = 2.02226624879595063154e-21, pio2_3_rempio = 2.02226624871116645580e-21, pio2_3t_rempio = 8.47842766036889956997e-32;

static const double one_kcos = 1.0, C1_kcos = 4.16666666666666019037e-02, C2_kcos = -1.38888888888741095749e-03, C3_kcos = 2.48015872894767294178e-05, C4_kcos = -2.75573143513906633035e-07, C5_kcos = 2.08757232129817482790e-09, C6_kcos = -1.13596475577881948265e-11;

static const double half_ksin = 0.5, S1_ksin = -0.16666666666666324348, S2_ksin = 8.33333333332248946124e-03, S3_ksin = -1.98412698298579493134e-04, S4_ksin = 2.75573137070700676789e-06, S5_ksin = -2.50507602534068634195e-08, S6_ksin = 1.58969099521155010221e-10;
...
double copysign_double(double x, double y) {
  __uint32_t hx, hy;
  ieee_double_shape_type gh_u_x, gh_u_y;
  gh_u_x.value = x;
  hx = gh_u_x.parts.msw;
  gh_u_y.value = y;
  hy = gh_u_y.parts.msw;
...
recompute:
  ...
  for (i = 0, j = jz, z = q[jz]; j > 0; i++, j--) {
    fw = (double)((int32_t)(twon24_krempio * z));
    iq[i] = (int32_t)(z - two24_krempio * fw);
    z = q[j - 1] + fw;
  }
  ...
  if (z == 0.0) {
    ...
    if (z >= two24_krempio) {
      fw = (double)((int32_t)(twon24_krempio * z));
      iq[jz] = (int32_t)(z - two24_krempio * fw);
      jz += 1;
      q0 += 24;
      iq[jz] = (int32_t)fw;
    } else
...
__int32_t __ieee754_rem_pio2(double x, double *y) {
  ...
  if (ix <= 0x413921fb) {
    ...
    if (n < 32 && ix != npio2_hw_rempio[n - 1]) {
      ...
      j = ix >> 20;
      y[0] = r - w;
      ieee_double_shape_type gh_u_y0;
      gh_u_y0.value = y[0];
      high = gh_u_y0.parts.msw;
      i = j - ((high >> 20) & 0x7ff);
      if (i > 16) {
        t = r;
        w = fn * pio2_2_rempio;
        ...
        y[0] = r - w;
        ieee_double_shape_type gh_u_y0_2;
        gh_u_y0_2.value = y[0];
        high = gh_u_y0_2.parts.msw;
        i = j - ((high >> 20) & 0x7ff);
        if (i > 49) {
          t = r;
          w = fn * pio2_3_rempio;
  ...
  ieee_double_shape_type sh_u;
  sh_u.value = z;
  sh_u.parts.msw = (ix - ((int32_t)e0 << 20));
  z = sh_u.value;
...
int main() {
  // The function shall return 1.0 if the argument is +-0.
  double x = 0.0;  // Replace nondeterministic input with a fixed value

  if (x == 0.0 || x == -0.0) {
    double res = cos_double(x);
    // x is +-0, the result shall be 1.0
    assert(res == 1.0);  // Use assert for verification
  }
