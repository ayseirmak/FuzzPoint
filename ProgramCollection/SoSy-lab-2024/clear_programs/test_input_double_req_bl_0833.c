...
int main() {
  // Set a deterministic value for testing `exp` with -INF.
  double x = -1.0 / 0.0; // -INF
  double res = __ieee754_exp(x);

  // Using assert for error handling, simulating 'reach_error' conditions.
  assert(res == 0.0 && __signbit_double(res) == 0);

  printf("Test passed: __ieee754_exp(-INF) returns +0.0\n");
