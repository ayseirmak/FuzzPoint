...
int main() {
    // Determine if -Inf results in NaN when passed to __ieee754_log10
    double x = -1.0 / 0.0; // -Inf
    double res = __ieee754_log10(x);

    // x is -Inf, result should be NaN
    if (!isnan_double(res)) {
        printf("Error: Expected NaN, but received a different result.\n");
        return 1;
    }

    printf("Passed: The result is NaN as expected.\n");
    return 0;
}