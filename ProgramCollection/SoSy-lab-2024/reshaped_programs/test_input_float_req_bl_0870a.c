...
float copysign_float(float x, float y) {
  __uint32_t ix, iy;
  ieee_float_shape_type gf_u1, gf_u2, sf_u;
  gf_u1.value = x;
  ix = gf_u1.word;
  gf_u2.value = y;
  iy = gf_u2.word;
...
float fabs_float(float x) {
  __uint32_t ix;
  ieee_float_shape_type gf_u, sf_u;
  gf_u.value = x;
  ix = gf_u.word;
...
static const float bp_pow[] = {1.0, 1.5},
                   ...
                   dp_l_pow[] = {0.0, 1.56322085e-06}, zero_pow = 0.0,
                   one_pow = 1.0, two_pow = 2.0, two24_pow = 16777216.0,
...
float __ieee754_powf(float x, float y) {
  ...
  s = one_pow;
  if ((((unsigned int)hx >> 31) - 1) == 0 && (yisint - 1) == 0)
    s = -one_pow;
  ieee_float_shape_type gf_u;
  gf_u.value = y;
  is = gf_u.word;
...
int main() {
  ...
  float x = 2.0f; // Fixed value for deterministic behavior
