...
// Union to convert between double and two 32-bit ints
typedef union {
    double value;
    struct {
        __uint32_t lsw;
...
double __ieee754_pow(double x, double y) {
    ...
    j = ew_u.parts.msw;
    if (j >= 0x40900000) {
        if (((j - 0x40900000) | ew_u.parts.lsw) != 0)
        ...
        if (((j - 0xc090cc00) | ew_u.parts.lsw) != 0)
...
int main() {
    ...
    double x = 0.5;  // Deterministic x value as 0 is neither less than 1 nor it will show error
    double y = -1.0 / 0.0;  // Set y to -Inf
    ...
    if (fabs_double(x) < 1.0) {
        ...
        if (!isinf_double(res)) {
            // If the assert fails, it prints an error
            fprintf(stderr, "Error: Expected result -inf but got a different value\n");
            return 1;
        }
        printf("Success: x = %.2lf, y = %lf, Result = %lf\n", x, y, res);
    } else {
        printf("Condition not met: |x| < 1.0\n");
    }
