...
// Function to calculate the square root of a float
float __ieee754_sqrtf(float x) {
  ...
  hx = ix & 0x7fffffff; // Ignore the sign bit

  // Special handling for non-finite numbers (Inf or NaN)
  if (hx >= 0x7f800000L) {
    return x * x + x;
  }
  ...
  if (hx == 0) return x;
  if (ix < 0) return (x - x) / (x - x); // NaN for negative input
...
float __ieee754_hypotf(float x, float y) {
  ...
  ieee_float_shape_type gf_u1, gf_u2;
  gf_u1.value = x;
  ha = gf_u1.word;
  gf_u2.value = y;
  hb = gf_u2.word;
...
// Function to check if a float value is infinity
...
int main() {
  // Hypot function test for infinity
  float x = 1.0f / 0.0f; // INF
  float y = 2.0f;        // Fixed test value
  ...
  printf("Hypot function test passed.\n");
  return 0;
}