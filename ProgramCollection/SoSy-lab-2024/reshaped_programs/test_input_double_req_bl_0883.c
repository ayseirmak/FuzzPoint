...
#include <math.h> // For INFINITY definition
...
int main() {
  /* A fixed value for y which adheres to the condition y > 0.0 */
  double x = INFINITY; // Assign +Inf to x
  double y = 2.0; // Use a positive fixed value for y

  if (y > 0.0) {
    double res = __ieee754_pow(x, y);

    // Check if the result is +Inf
    if (!isinf_double(res)) {
      printf("Error: Expected result to be +Inf.\n");
      return 1;
    }
  }
  printf("Test passed: Power function returned +Inf as expected.\n");
