#include <stdio.h>
#include <math.h> // For INFINITY constant and mathematical operations
...
// Utility functions for absolute value calculation
...
int main() {
  ...
  double y = -1.0; // Deterministic input for y
  ...
  if (isinf_double(x) && isfinite_double(y) && y < 0.0) {
    ...
    if (res != -pi) {
      printf("Error: Expected result is -pi, got %.12f\n", res);
      return 1;
    } else {
      printf("Test passed: Result is -pi as expected.\n");
    }
  }
  return 0;
}