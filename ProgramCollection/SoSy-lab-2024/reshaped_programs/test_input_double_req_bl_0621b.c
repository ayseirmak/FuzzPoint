...
/* Constants used in the atan_double function */
...
int main() {
  ...
  double x = -1.0 / 0.0; // Represents negative infinity
  double res = atan_double(x);

  // If x is +Inf or -Inf, the result should be +-pi/2 (only in this test, x was set to -Inf)
  assert(res == -pi_o_2);
