...
// Ensures compatibility with ARM and other architectures
...
int main() {
  // Deterministic input
  double x = -0.0;
  double y = -1.0;  // Fixed value for y instead of nondeterministic input
  ...
  if (y < 0.0) {
    ...
    if (res != -pi_o_2) {
      printf("Error: Expecting result to be -pi/2, but got %f\n", res);
      return 1;
    }
  }

  printf("Test passed: y=%f, x=%f, result is -pi/2 as expected.\n", y, x);
  return 0;
}