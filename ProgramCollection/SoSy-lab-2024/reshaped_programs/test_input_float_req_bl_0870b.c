...
int main() {
    float x = NAN; // Define x as NAN explicitly.

    // Fixed value for y since non-determinism is not allowed
    float y = 2.0; // Replace with any fixed value other than 0.0f to test.

    if (y != 0.0f) {
        float res = __ieee754_powf(x, y);
        if (!isnan_float(res)) {
            printf("Error: Expected NaN but got a valid result.\n");
            assert(0);
        } else {
