...
// Define all tensor weights and biases here (snipped for brevity since they are lengthy)
// They should match the original program for correct operation

// Function declarations
void entry(const float tensor_input[1][4], float tensor_output[1][2]);

int main() {
    // Initialized deterministic input within specified range
    float tensor_input[1][4] = {
        {-0.246584, 0.140568, -0.03516, -0.592776} // Use values in the allowed range for testing
    };
    float tensor_output[1][2];

    entry(tensor_input, tensor_output);

    // Assert condition automatically
    assert(!(tensor_output[0][0] <= tensor_output[0][1]));

    return 0;
}

// Node functions, identical in logic to the provided specification
