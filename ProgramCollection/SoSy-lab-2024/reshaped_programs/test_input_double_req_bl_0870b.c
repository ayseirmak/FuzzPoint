...
static const double zero_pow = 0.0, one_pow = 1.0, two_pow = 2.0,
    ...
    tiny_pow = 1.0e-300, L1_pow = 5.99999999999994648725e-01,
...
double __ieee754_pow(double x, double y) {
    ...
    if (iy > 0x41e00000) {
        ...
        t_h = zero_pow;
        ieee_double_shape_type sh_u2;
        sh_u2.value = t_h;
        sh_u2.parts.msw = (((ix >> 1) | 0x20000000) + 0x00080000 + (k << 18));
        t_h = sh_u2.value;
        t_l = ax - (t_h - bp_pow[k]);
    ...
    if (i > 0x3fe00000) {
        ...
        t = zero_pow;
        ieee_double_shape_type sh_u3;
        sh_u3.value = t;
        sh_u3.parts.msw = n & ~(0x000fffff >> k);
        t = sh_u3.value;
...
int main() {
    ...
    double y = 1.23; // Some fixed non-zero value

    if (y != 0.0 && y != -0.0) {
        double res = __ieee754_pow(x, y);
        if (!isnan_double(res)) {
            printf("Error: Expected NaN but got a different result.\n");
            return 1;
        } else {
            printf("Success: Result is NaN as expected.\n");
        }
    } else {
        printf("y is zero, skipping calculation.\n");
    }
