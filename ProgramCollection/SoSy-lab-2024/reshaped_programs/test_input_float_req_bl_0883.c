...
float __ieee754_powf(float x, float y) {
  ...
  if (iy > 0x4d000000) {
    ...
    s2 = s * s;
    r = s2 * s2 *
        (L1_pow + s2 * (L2_pow + s2 * (L3_pow + s2 * (L4_pow + s2 * (L5_pow + s2 * L6_pow)))));
    r += s_l * (s_h + s);
    s2 = s_h * s_h;
    t_h = 3.0f + s2 + r;
    gf_u.value = (t_h);
...
int main() {
  ...
  float x = INF; // Fixed value of infinity
  float y = 2.0f; // Deterministic input value
