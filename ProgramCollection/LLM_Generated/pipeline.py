...
def extract_c_files_with_floats_simple(clone_dir):
    file_text_path = "c_files_list.txt"
    pattern = re.compile(r'\b(float|double|long double)\b')
    c_files = []
    b_files = []
    for root, _, files in os.walk(clone_dir):
        for file in files:
...
if __name__ == "__main__":
    ...
    #CLONE_DIR = "/users/a_irmak/FloatingPoint_and_CompilerTesting/Program_Generation_GPT4o/"
    #c_files = extract_c_files_with_floats_simple(CLONE_DIR)
