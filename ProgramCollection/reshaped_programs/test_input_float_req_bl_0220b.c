...
float scalbn_float(float x, int n) {
  ...
  if (!((hx) < 0x7f800000L))
    return x + x;
  if (((hx) < 0x00800000L)) {
    x *= 3.355443200e+07; // two25_scalbn
    gf_u.value = x;
    ix = gf_u.word;
    k = ((ix & 0x7f800000) >> 23) - 25;
    if (n < -50000)
      return 1.0e-30 * x; // tiny_scalbn
  }
  k = k + n;
  if (k > (0x7f7fffffL >> 23))
    return 1.0e+30 * copysign_float(1.0e+30, x); // huge_scalbn
  if (k > 0) {
    ieee_float_shape_type sf_u;
  ...
  if (k < -22) {
    if (n > 50000)
      ...
      return 1.0e-30 * copysign_float(1.0e-30, x); // tiny_scalbn
  }
  k += 25;
  ieee_float_shape_type sf_u;
  sf_u.word = ((ix & 0x807fffff) | (k << 23));
  x = sf_u.value;
  return x * 2.9802322388e-08; // twom25_scalbn
}
