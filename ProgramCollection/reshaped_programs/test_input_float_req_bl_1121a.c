...
float fmod_float(float x, float y) {
    ...
    if (hx < hy) return x;
    if (hx == hy) return Zero_fmod[(__uint32_t)sx >> 31];
    ...
    while (n--) {
        ...
        if (hz < 0) {
            ...
            if (hz == 0) return Zero_fmod[(__uint32_t)sx >> 31];
            hx = hz + hz;
        }
    }
    ...
    if (hx == 0) return Zero_fmod[(__uint32_t)sx >> 31];
...
int main() {
    float x = 3.14f; // Deterministic input for testing
    float y = 0.0f;

    float res = fmod_float(x, y);

    // x is any value, y is +-0, the result shall be NaN
    assert(isnan_float(res) && "REACH_ERROR");
