...
// Function to compute the modulus of a double value
double fmod_double(double x, double y) {
  __int32_t n, hx, hy, hz, ix, iy, sx, i;
  __uint32_t lx, ly, lz;

  // Extract higher and lower 32 bits of both numbers
  do {
  ...
  // Return NaN if y is 0 or x is NaN
...
int main() {
  // Set deterministic numbers for testing
  double y = -1.0 / 0.0; // Infinity
  double x = 1.2345; // Arbitrary finite value for testing
  ...
  if (isfinite_double(x)) {
    ...
    assert(res == x);
    printf("Test passed: x=%.4f, y=%.4f, result=%.4f\n", x, y, res);
  }
