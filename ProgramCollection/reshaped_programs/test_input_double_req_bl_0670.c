...
double atan_double(double x) {
    ...
    s2 =
        w * (aT_atan[1] + w * (aT_atan[3] + w * (aT_atan[5] + w * (aT_atan[7] + w * aT_atan[9]))));
    if (id < 0)
        return x - x * (s1 + s2);
    else {
...
int main() {
    double x = NAN; // Deterministic test value 1
    double y = 4.0; // Deterministic test value 2
