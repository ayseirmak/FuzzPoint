...
int main() {
  float x = 0.0f;
  // Fix the y value to a known static negative value
  float y = -1.0f;

  // REQ-BL-0662: The atan2 and atan2f procedures shall return -pi/2,
  // if y is < 0 and x is +-0
  if (y < 0.0f) {
    float res = __ieee754_atan2f(y, x);

    // Assert the result to be -pi/2 as per the requirement
    assert(res == -pi_o_2);
  }
