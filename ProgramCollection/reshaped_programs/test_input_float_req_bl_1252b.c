...
// Categorizes floating point value into categories like zero, subnormal, etc.
...
// Determines the sign of the float (positive or negative)
...
int main() {
  ...
  if (!(res == 0.0f && __signbit_float(res) == 0)) {
    assert(0 && "Error: Expected result to be +0");
    return 1;
  }

  printf("Test passed: fmax(+0, -0) = +0\n");
  return 0;
}