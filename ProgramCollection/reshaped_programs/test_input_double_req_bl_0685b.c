...
double atan_double(double x) {
    ...
    if (ix < 0x3fdc0000) {  // |x| < 0.4375
        ...
        if (ix < 0x3ff30000) {  // |x| < 1.1875
            ...
            if (ix < 0x40038000) {  // 1.1875 <= |x| < 2.4375
                id = 2;
...
double __ieee754_atan2(double y, double x) {
    ...
    switch (m) {
        case 0:
            return z;
        case 1: {
            ieee_double_shape_type zh_u;
            zh_u.value = z;
            zh_u.parts.msw ^= 0x80000000;
            return zh_u.value;
        }
        case 2:
...
int main() {
    ...
    if (res != -3 * PI_O_4) {
        printf("Error: Expected -3pi/4, got %f\n", res);
        return 1;
    }
    printf("Result is -3pi/4, as expected.\n");
    return 0;
}