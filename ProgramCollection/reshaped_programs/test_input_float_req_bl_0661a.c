...
static const float one_atan = 1.0f, huge_atan = 1.0e30f, pi_o_4 = 7.8539818525e-01f, pi_o_2 = 1.5707963705e+00f, pi = 3.1415927410e+00f;
...
static const float tiny_atan2 = 1.0e-30f, zero_atan2 = 0.0f, pi_lo_atan2 = -8.7422776573e-08f;
...
int main() {
  float x = 1.0f; // Deterministic, fixed value for x
  float y = -0.0f;

  if (x > 0.0f) {
    float res = __ieee754_atan2f(y, x);
    if (res != -0.0f) {
      printf("Error: expected -0, got %f\n", res);
      return 1;
    }
  }

  printf("Test passed: atan2f(-0.0, x > 0) returns -0.0 as expected.\n");
  return 0;
}