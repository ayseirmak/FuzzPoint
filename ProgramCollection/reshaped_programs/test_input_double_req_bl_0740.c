...
double __ieee754_sqrt(double x) {
    ...
    if (ix0 <= 0) {
        if (((ix0 & (~sign)) | ix1) == 0) {
            return x; // return sqrt(+-0) = +-0
        } else if (ix0 < 0) {
            return (x - x) / (x - x); // return a NaN
        }
    }
    ...
    // Use standard IEEE 754 add to add tiny to ensure exact rounding according to the IEEE standard.
    if ((ix0 | ix1) != 0) {
...
int main() {
    // Deterministic input: fixed value of -INF
    double x = -1.0 / 0.0; // -INF
    double res = __ieee754_sqrt(x);

    // x is -INF, the result should be NaN
    if (!isnan_double(res)) {
        printf("Error: Result is not NaN when input is -INF.\n");
        return 1;
    }

    printf("Test passed: Result is NaN as expected when input is -INF.\n");
    return 0;
}