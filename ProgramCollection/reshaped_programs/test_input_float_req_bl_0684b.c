...
float __ieee754_atan2f(float y, float x) {
    ...
    if ((ix > 0x7f800000U) || (iy > 0x7f800000U))
        return x + y;
...
int main() {
    float x = 1.0f; // Deterministic input for x
    float y = -INFINITY; // -Inf represents negative infinity

    if (isfinite_float(x)) {
        float res = __ieee754_atan2f(y, x);

        // x is finite, y is -inf, the result should be -pi/2
        assert(res == -pi_o_2);
    }
