...
int main() {
  ...
  // Predefined deterministic input for testing the case outside the range [-1,1]
  x = 1.5; // A specific test value outside the range [-1,1]
  ...
  if (x < -1.0 || x > 1.0) {
    ...
    if (!isnan_double(res)) {
      printf("Error: The computed result is not NaN for x = %.2f\n", x);
      return 1;
    } else {
      printf("Success: The computed result is NaN for x = %.2f\n", x);
    }
  }
