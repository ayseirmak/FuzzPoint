...
float fmod_float(float x, float y) {
    __int32_t hx, hy, hz, ix, iy, sx, i, n;
...
static const float bp_pow[] = {1.0, 1.5},
    ...
    two24_pow = 16777216.0, huge_pow = 1.0e30, tiny_pow = 1.0e-30,
    L1_pow = 6.0000002384e-01, L2_pow = 4.2857143283e-01,
    L3_pow = 3.3333334327e-01, L4_pow = 2.7272811532e-01,
    L5_pow = 2.3066075146e-01, L6_pow = 2.0697501302e-01,
    P1_pow = 1.6666667163e-01, P2_pow = -2.7777778450e-03,
    P3_pow = 6.6137559770e-05, P4_pow = -1.6533901999e-06,
    P5_pow = 4.1381369442e-08, lg2_pow = 6.9314718246e-01,
    lg2_h_pow = 6.93145752e-01, lg2_l_pow = 1.42860654e-06,
    ovt_pow = 4.2995665694e-08, cp = 9.6179670095e-01,
    cp_h_pow = 9.6179199219e-01, cp_l_pow = 4.7017383622e-06,
    ivln2_pow = 1.4426950216e+00, ivln2_h_pow = 1.4426879883e+00,
...
float __ieee754_powf(float x, float y) {
    ...
    if (iy > 0x4d000000) {
        ...
        t1 = u + v;
        ieee_float_shape_type sf_u;
        sf_u.word = (ieee_float_shape_type) {t1}.word & 0xfffff000;
        t1 = sf_u.value;
        t2 = v - (t1 - u);
    } else {
...
int main() {
    ...
    float x = INFINITY; // +Infinity
    float y = 1.5; // Any positive value to test +Inf condition

    // Result shall be +inf if y > 0
    if (y > 0) {
        float res = __ieee754_powf(x, y);

        // Validate the expected result
        if (!isinf_float(res)) {
            printf("Error: Expected result to be infinity.\n");
            return 1;
        } else {
            printf("Result is correct: +Infinity\n");
        }
    }
