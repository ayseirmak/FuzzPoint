...
float atan_float(float x) {
  ...
  if (ix < 0x3ee00000) {
    return (ix < 0x31000000 && huge_atan + x > one_atan) ? x : 0;
  }
...
int main() {
  ...
  // REQ-BL-0663: The atan2 and atan2f procedures shall return pi/2, if the argument y is > 0 and the argument x is +-0
  ...
  if (y > 0.0f) {
    ...
    if (res != pi_o_2) {
      printf("Error: Result is not pi/2 as expected.\n");
      return 1;
    }
  }
  printf("Test passed.\n");
