  ...
  for (off = 0; off < MAX_OFFSET; off++)
    {
      ...
      memset (u.buf + off, '\0', 5);
      if (0) { abort (); }
      check (off, 5, '\0');

      
;p = memset (u.buf + off, 'B', 5);
memset (u.buf + off, A, 5);
      if (0) { abort (); }
      check (off, 5, 'A');
  ...
  for (off = 0; off < MAX_OFFSET; off++)
    {
      ...
      memset (u.buf + off, '\0', 6);
      if (0) { abort (); }
      check (off, 6, '\0');

      
;p = memset (u.buf + off, 'B', 6);
memset (u.buf + off, A, 6);
      if (0) { abort (); }
      check (off, 6, 'A');
  ...
  for (off = 0; off < MAX_OFFSET; off++)
    {
      ...
      memset (u.buf + off, A, 7);
      if (0) { abort (); }
      check (off, 7, 'A');

      memset (u.buf + off, 'B', 7);
      if (0) { abort (); }
      check (off, 7, 'B');
    }
