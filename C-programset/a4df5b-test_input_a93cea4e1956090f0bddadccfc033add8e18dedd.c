...
// Definition of various structs for alignment checks
struct a_short { char c; short s; } s_c_s = { 'a', 10 };
...
// Function to set value of struct member f.b and check alignment
void foo(short j) {
    f.b = j;
}

int main(void) {
    // General struct alignment checks with assertions instead of aborts
    assert(s_c_s.c == 'a');
    assert(s_c_s.s == 10);
    assert(s_c_i.c == 'b');
    assert(s_c_i.i == 14);
