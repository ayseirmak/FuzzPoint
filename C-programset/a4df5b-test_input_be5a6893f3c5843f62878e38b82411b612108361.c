...
#define A(x) do { if (!(x)) abort(); } while (0)
...
static struct et6
{
    ...
    unsigned int nv6;  // Corrected from `lo` and assumed name
    float ov6;         // Assumed to require a float and name
    float pv6;
    float qv6;
    int rv6;
    union ct6 {
    ...
    1366022414U, 1.0f, 1.0f, 1.0f, 305,  // Corrected values and added floats
    { 1069379046L }, NULL,               // Initialized pointer
    { 3318.041978 }
};
...
{
    union at6 bav6 = callee_af6(wv6, xv6, yv6, zv6);
    (void)bav6; // To suppress unused variable warning
}
...
static struct et7
{
    ...
    float lv7[0];      // Flexible array member
    signed mv7:9;
    short int nv7;
    double ov7;
    float pv7;
...
static long long int bdv7 = 38178602L;  // Corrected syntax
